...
const Toast: React.FC<ToastProps> = ({ message, type = 'info', duration = 5000, error, onClose }) => {
  ...
  const bgColorClass = type === 'success' ? 'bg-green-500' : type === 'error' ? 'bg-red-500' : type === 'loading' ? 'bg-yellow-500' : 'bg-blue-500';

  const iconClass = type === 'success' ? 'text-green-500' : type === 'error' ? 'text-red-500' : type === 'loading' ? 'text-yellow-500' : 'text-blue-500';

  return (
    <div
      className={`flex flex-col p-4 mb-2 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 transition-all duration-300 ease-in-out ${expanded ? 'max-w-md' : ''}`}
      role="alert"
    >
      <div className="flex items-center w-full">
        <div className={`inline-flex items-center justify-center shrink-0 w-8 h-8 ${iconClass} bg-gray-100 rounded-lg dark:bg-gray-700`}>
          {type === 'success' && (
